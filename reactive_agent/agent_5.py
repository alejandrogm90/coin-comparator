...
if __name__ == '__main__':
    if len(sys.argv) != 6:
        ...
        if currentPrice > previousPrice and myBank.getCash() > currentPrice and Bank1.isDecreasing(rows) and \
            myBank.getBuyGanances(myBank.getCash()) > myBank.getBuyTaxeAmount(myBank.getCash()):
            myBank.buyAllCoins()
        elif currentPrice < previousPrice and myBank.getCoins() > 0 and Bank1.isIncreasing(rows) and \
